import { Component, OnInit, OnChanges, AfterViewInit, SimpleChanges, Input, ChangeDetectorRef, ElementRef, ViewChild, EventEmitter } from '@angular/core';
...
export class BellCurveComponent implements OnChanges, OnInit, AfterViewInit {
    ...
    plugins: {
      datalabels: {
        formatter: (value, context) => {
          const total = this.doughnutChartData.datasets[0].data.reduce((a, b) => a + b, 0);
  ...
  resetDashboard() {
    ...
    if (this.bellCurveChart) {
      this.bellCurveChart.data.datasets[0].data = Array(this.bellCurveLabels.length).fill(0);
  ...
  ngAfterViewInit() {
    ...
    setData(chart: any, data = []) {
      ...
      const scoreRanges = this.calculateScoreRanges(this.dashboardData, this.scoreType);

      const labels = this.scoreType === '' ? this.chartLabels : this.chartLabels_ScoreType;
  ...
  calculateScoreRanges(data: any[], scoreType: string): any {
    const ranges = { '0-39': 0, '40-49': 0, '50-59': 0, '60-69': 0, '70-79': 0, '80+': 0 };

    if (Array.isArray(data)) {
        const studentData: any[] = data.find(item => Array.isArray(item.studentScore))?.studentScore || [];
        ...
        studentData.forEach(student => {
            ...
            if (scoreType === 'คะแนนกลางภาค') {
                ...
                totalScore = (student.accumulated_score || 0) + (student.midterm_score || 0) + (student.final_score || 0);
            }
